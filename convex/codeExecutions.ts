...
export const saveExecution = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const saveCodeExecutions = { ...args, userId: identity.subject };
    console.log(saveCodeExecutions);
    await ctx.db.insert("codeExecutions", saveCodeExecutions);
  },
});
...
export const getUserStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const snippetIds = starredSnippets.map((star) => star.snippetsId);
    const snippetDetails = await Promise.all(
      snippetIds.map((id) => ctx.db.get(id))
    );

    //calculate most starrted language
    const starredLanguage = snippetDetails.filter(Boolean).reduce(
      (acc, curr) => {
        if (curr?.language) {
    ...
    const mostStarredLanguage =
      Object.entries(starredLanguage).sort(([, a], [, b]) => b - a)[0]?.[0] ??
      "N/A";

    // Calculate execution stats
    const last24Hours = executions.filter(
      (e) => e._creationTime > Date.now() - 23 * 60 * 60 * 1000
    ).length;
